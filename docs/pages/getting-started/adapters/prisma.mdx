...
```bash npm2yarn
npm install @prisma/client @auth/prisma-adapter @prisma/adapter-pg dotenv pg
...
This will create `prisma/schema.prisma`, `prisma.config.ts` and a `.env` file containing a local `DATABASE_URL`. The `src/generated/prisma` output directory for the generated Prisma Client will be created when you run `prisma generate` or `prisma migrate dev` in a later step.

Next, create a database using your preferred method and replace the generated `DATABASE_URL` in your `.env` file with the `postgresql://...` connection string from the CLI output. For example, the following command creates a Prisma Postgres database:

```bash
npx create-db
```

If your `DATABASE_URL` starts with `postgres://` instead of `postgresql://`, change it to `postgresql://` to make Prisma migrations work correctly. Note that you should put the `DATABASE_URL` in the `.env` file and not in `.env.local`.

<Callout type="info">
  If you are deploying this on Vercel, you should add the environment variable
  `DATABASE_URL` in your Vercel project settings as well.
</Callout>

Create a `prisma.ts` file with the following code:
...
This file creates a Prisma Client and attaches it to the global object so that only one instance of the client is created in your application. The exported `prisma` instance can then be imported and used in your application and passed to the Prisma Adapter.

If you are seeing error squiggles under `"@/src/generated/prisma/client"`, that is because we haven't generated the Prisma Client yet. You can ignore this for now and it should go away after you run `prisma generate` in a later step.

<Callout type="info">
  If you are using something else instead of Postgres (like MySQL, SQLite,
  etc.), install the corresponding driver adapter package instead of
...
Update `auth.ts` to use the Prisma Adapter and the `prisma` instance you just created. Make sure to fill the `providers` array with the providers you are using.
...
Run the following command to generate the Prisma Client in the `src/generated/prisma` directory. Don't forget to add this directory to your `.gitignore` file if you have one.
...
<Callout type="info">
  If you are deploying this on Vercel, you should update your build command to
  run `prisma generate` before the build step. For example, you can set your
  build command to `npm exec prisma generate && next build` if you are using
  Next.js.
</Callout>

That's it! You should now have the Prisma Adapter set up and ready to use in your application.
